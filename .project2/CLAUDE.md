...
`.project2/` 경량 하네스의 진입점. Phase 2에서 `.project2/`가 `.claude/`로 이동하면 경로를 갱신한다.
...
| 작업 프로세스 | `.project2/skills/workflow.md` |
| 설계·계획 | `docs/superpowers/{specs,plans}/` |

하네스 폴더 `.project2/`: `agents/` `rules/` `hooks/` `skills/` `commands/`. `skills/`만 내용이 있고 나머지는 **의도적으로 비어 있다**. 각 폴더는 그 기능이 실제로 필요해질 때 항목 하나씩 사용자 승인 후 채운다 (§6).

## 5. Skill & Agent Routing

작업 프로세스와 서브에이전트 할당 규칙은 `.project2/skills/workflow.md`가 정한다. 요약:
